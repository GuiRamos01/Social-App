...
const Post = ({ post }) => {
  ...
  const [fixedPost, setIsFixed] = useState(false);
  ...
  const [repostPetition, setRepostPetition] = useState([]);

  useEffect(() => {
    if (post.repostPetition) {
      const fetchPosts = async () => {
        try {
          const res = await axios.get(`/api/petitions/pub/${post.repostPetition}`);
          setRepostPetition(res.data);
        } catch (error) {
          console.error("Erro ao buscar posts:", error);
        }
      };
  ...
  const handleRepost = async () => {
    try {
      await axios.post(`/api/posts/repost/create/${post._id}`, { shortLink: undefined, type: 4 });
  ...
  useEffect(() => {
    if (currentUser && currentUser._id === post.userId) {
      const fetchPosts = async () => {
        try {
          if (post.groupId !== "0") {
            const res = await axios.get(`/api/groups/block-check/${post.groupId}/${post.userId}`);
  ...
  return (isAvailable && (
    <div id="Post" className={post.status === "edited" && ("EditedPost")}>
      <div className="headerPost">  
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            {currentUser && post.status === "on" && post.groupId < 1 &&
              post.userId === currentUser._id && (
                fixedPost  ? (
      ...
      {post.type === 4 ? (
        <div>
          <span className="repostIcon gap-1 col-2">
          ...
          {post.repostPetition && (
            <PetitionPost key={repostPetition._id} post={repostPetition} />
          )}
          ...
          {post.text && (post.type === 1) && (
            isTranslate ? (
              <div className="translate">
                <div>
                  <p>{translatedText}</p>
                  <h1 className="mb-2">{t("Traduzido automaticamente")}</h1>
                </div>
              </div>
            ):(<>
              <Expandable>
              ...
              {isSubmitting && <Loader2 className="mr-1 mb-3 h-4 w-4 col-3 animate-spin" />}
            </>)
          )}
            ...
            {videoPublish.status === "on" ? (
              <div>
                <div key={1} className="pVideo">
                  ...
                  <div className="thumbnail">
                    <img src={videoPublish.videoThumb} alt={`Thumbnail ${0}`} className="thumbImage" />
                  </div>
                </div>

                <div className="msgVideo2">
                  <Link to={`/${videoPublish.username}/post${videoPublish._id}`}>
                    <span>
        ...
        <div className="postFooter col-2">
          {isLike ? (
              <div onClick={handleUnlike} className="pfItem pfItem2 col-1">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6">
            ...
            user1.language !== user.language && (
              isTranslate ? (
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger>
                      <div onClick={removeTranslate} className="pfItem pfItem2">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                    ...
                    <TooltipContent>
                      <p className="tooltip">{t("Ver original")}</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              ):(
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger>
                      <div onClick={translateText} className="pfItem pfItem2">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
                          <path strokeLinecap="round" strokeLinejoin="round" d="m10.5 21 5.25-11.25L21 21m-9-3h7.5M3 5.621a48.474 48.474 0 0 1 6-.371m0 0c1.12 0 2.233.038 3.334.114M9 5.25V3m3.334 2.364C11.176 10.658 7.69 15.08 3 17.502m9.334-12.138c.896.061 1.785.147 2.666.257m-4.589 8.495a18.023 18.023 0 0 1-3.827-5.802" />
                        </svg>
                      </div>
                    ...
                    <TooltipContent>
                      <p className="tooltip">{t("Traduzir post")}</p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              )
