...
const handleUnblock = async () => {
  ...
  return (
    <aside id="Profile" className="Container">
      ...
      {user && user.name && (
        <Helmet>
          <title>{user.name} / SocialApp</title>
        </Helmet>
      )}
            ...
            <div className="hu-details2 col-1">
              ...
              {user.typeUser !== 3 && (
                <div className="hu-buttons">
                  {isFollowing ? (
                      <button className="bor-1 col-1" onClick={handleUnfollow} disabled={isBlockButton}>{t('Seguindo')}</button>
                      ):(
                      <button className="bor-1 col-1" onClick={handleFollow} disabled={isBlockButton}>{t('Seguir')}</button>
                  )}
