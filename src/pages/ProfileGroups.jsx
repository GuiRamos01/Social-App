...
const ProfileGroups = () => {
  ...
  useEffect(() => {
    if (user._id) {
      const fetchGroup = async () => {
        try {
          const res = await axios.get(`/api/groups/user/${user._id}`);
          setGroups(res.data);
        } catch (error) {
  ...
  return (
    <aside id="GroupsUser" className="Container">
      ...
      {user && user.username && (<Helmet>
        <title>{t("Grupos de")} @{user.username} / SocialApp</title>
      </Helmet>)}
      ...
      <div className="Main">
        ...
        <div className="w-full">
          <h1 className="title col-2">{t("Grupos de")} @{user.username}</h1>
