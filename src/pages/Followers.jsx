...
const Followers = () => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const res = await axios.get(`/api/users/username/${encodedInput}`);
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const res = await axios.get(`/api/users/followers/${encodedInput}`);
        setItem(res.data);
      } catch (error) {
  ...
  return (
    <aside id="Search" className="Container SearchMenu">
      {user && user.name && (
        <Helmet>
          <title>{t("Seguidores")} - {user.name} / SocialApp</title>
        </Helmet>
      )}
      ...
      <div className="Header headerMenu bg-1 col-1">
        <Link to={`/${encodedInput}/followers`}><div className="Item itemActive">{t("Seguidores")}</div></Link>
        <Link to={`/${encodedInput}/following`}><div className="Item">{t("Seguindo")}</div></Link>
      </div>
